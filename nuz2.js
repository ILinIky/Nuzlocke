...
// Works with Netlify function '/.netlify/functions/nuzlocke' (see API_URL).
// If your backend is mounted at '/api/nuzlocke', change API_URL accordingly.
...
(async function init(){
  ...
  bindTabs();
  bindHeader();
  const li = document.getElementById('lobbyId');
  if (li) li.value = state.lobbyId;
  await refreshAll();
  setStatus('Bereit');
})();
